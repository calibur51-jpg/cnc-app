...
# 💡 2. 這裡請填入你剛剛上傳到 GitHub 的 JSON 憑證完整檔名！
JSON_FILE_NAME = "cnc-system-497409-64ccd035a4d1.json"
...
if df_inv.empty:
    st.warning("正在等待雲端資料庫連線設定...請確認憑證檔名是否填寫正確。")
else:
    # 頁面分頁
            ...
            if not operator:
                ...
                if action == "領用 (-)":
                    if current_stock < qty:
                        ...
                        ws_inv.update_cell(sheet_row, df_inv.columns.get_loc("Mercury0") + 1 if "Mercury0" in df_inv.columns else df_inv.columns.get_loc("目前庫存") + 1, new_stock)
